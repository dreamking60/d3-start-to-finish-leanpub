...
and JSON files are text files with a format closely related to JavaScript objects and arrays. For example:
...
For example, suppose you have some data that represents scores out of 10. This means the data range is `[0, 10]`. And suppose you want the length of the bars to vary from `0` to `500`, this means the output range is `[0, 500]`. You can create a suitable scale function using:
...
Since its inception in 2011, D3 has undergone a number of changes and at the time of writing is at version 6. This book uses **version 6** which was launched in August 2020.