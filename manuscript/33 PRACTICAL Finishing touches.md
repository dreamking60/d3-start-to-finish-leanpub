...
There’s a huge amount of resources and opinion surrounding font choice and unless you’re a trained designer it can be hard to know where to turn. I usually turn to Google Fonts and filter by Sans Serif and sort by most popular. Currently this returns Roboto, Open Sans and Lato as popular sans serif fonts. I usually try each of these and use my instinct to choose one. In the case of the Energy Explorer I’ve chosen Open Sans. Feel free to choose a different font if you prefer.
...
Save `index.html` and `style.css` and load `step15` in the browser. The font has changed to Open Sans:
...
We use the `.html` method instead of `.text` in `updateMenu` to update the content of each menu item using HTML instead of a text string. We pass in function `getHtml` which returns an HTML string consisting of an SVG circle and the label. Each circle is given the same class as its counterpart in the chart, so that it adopts the existing circle styles in `css/style.css`.
